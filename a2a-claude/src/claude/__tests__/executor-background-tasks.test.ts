...
const states = (events: PublishedEvent[]): string[] =>
  events.filter((e) => e.kind === "status-update").map((e) => (e.status?.state ?? "") as string);

const artifacts = (events: PublishedEvent[]): PublishedEvent[] =>
  events.filter((e) => e.kind === "artifact-update");
...
describe("held-open A2A task", () => {
  ...
  it("still publishes an artifact for an empty-text result carrying structured output", async () => {
    // Fork-only guard: `claude.outputFormat` puts the payload in a JSON data
    // part, so a round whose text is empty can still have something to
    // deliver. The empty-text skip above must not swallow it.
    const client = new FakeClaudeClient([{
      messages: [
        init("s1"),
    ...
    const published = artifacts(events);
    expect(published).toHaveLength(1);
    const parts = (published[0].artifact as { parts: Array<Record<string, unknown>> }).parts;
    expect(parts.find((p) => p.kind === "data")?.data).toEqual({ answer: "42" });
  });
  ...
  it("gives each round its own streaming artifact id and lastChunk marker", async () => {
    ...
    const ids = artifacts(events).map(
      (a) => (a.artifact as { artifactId?: string } | undefined)?.artifactId,
    );
    expect(ids).toEqual([
    ...
    const markers = artifacts(events).filter((a) => a.lastChunk === true);
